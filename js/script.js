	 	...
	 	success : function(data){
            ...
            $(location).attr('href',url);
             ...
             $("#error").text("Такой login уже существует. Выберите другой").removeClass("success").addClass("error").show().delay(3000).fadeOut(300);
                ...
                $("#error").text("Ошибка. Проверте соединение").removeClass("success").addClass("error").show().delay(3000).fadeOut(300);
            }
	 }
	 });
	}
            ...
            if(name_login == "" || pass_login == ""){
            	...
            	$.ajax({
            		...
            		success:function(data){
            			if(data){
            				if(data == 0) {
            					$("#error").text("Такой пары логин/пароль не существует. Возможно вы еще не зарегистрированы").removeClass("success").addClass("error").show().delay(3000).fadeOut(300);
            				}
            				else if(data == 1) {
 							...
 							$("#error").text("Данные не получены").removeClass("success").addClass("error").show().delay(3000).fadeOut(300);
            			}
